...
int *countMatches(int *seq1, int *seq2) {
  ...
  for (int i = 0; i < seqlen; i++){
    if (seq1[i] == seq2[i]){
      if (seq2[i] == approxValue && code[1] != 0){
        code[1]--;
      }
      code[0]++;
    }
      ...
      for (int j = 0; j < seqlen; j++){
        if (seq1[i] == seq2[j] && i != j && seq2[j] != approxValue){
          approxValue = seq2[j];
          code[1]++;
        }
      }
...
void strobe (const struct lcdDataStruct *lcd)
{
  ...
  writeLED (gpio, lcd->strbPin, 1) ; delayMicroseconds (50) ;
  writeLED (gpio, lcd->strbPin, 0) ; delayMicroseconds (50) ;
}
...
void sendDataCmd (const struct lcdDataStruct *lcd, unsigned char data)
{
  ...
  if (lcd->bits == 4)
  {
    ...
    for (i = 0 ; i < 4 ; ++i)
    {
      writeLED (gpio, lcd->dataPins [i], (d4 & 1)) ;
      d4 >>= 1 ;
    }
    strobe (lcd) ;
  ...
  else
  {
    for (i = 0 ; i < 8 ; ++i)
    {
      writeLED (gpio, lcd->dataPins [i], (myData & 1)) ;
      myData >>= 1 ;
    }
  }
  strobe (lcd) ;
...
void lcdPut4Command (const struct lcdDataStruct *lcd, unsigned char command)
{
  ...
  for (i = 0 ; i < 4 ; ++i)
  {
    writeLED (gpio, lcd->dataPins [i], (myCommand & 1)) ;
    myCommand >>= 1 ;
  }
  strobe (lcd) ;
}
...
{ // this is just a suggestion of some variable that you may want to use
  ...
  writeLED (gpio, lcd->rsPin,   0) ; pinMode (gpio, lcd->rsPin,   OUTPUT) ;
  writeLED (gpio, lcd->strbPin, 0) ; pinMode (gpio, lcd->strbPin, OUTPUT) ;

  for (i = 0 ; i < bits ; ++i)
  {
    writeLED (gpio, lcd->dataPins [i], 0) ;
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  while (!found) 
  {
    ...
    lcdClear(lcd);
    char attemptString[20];
    sprintf(attemptString, "Attempt: %d",attempts);
    lcdPosition(lcd, 0, 0);
    lcdPuts(lcd, attemptString);
    printf("\n");
    ...
    else if (attempts == 5)
    {
      ...
      char exactMatches[20], approxMatches[20];
      sprintf(exactMatches, "E: %d",code[0]);
      sprintf(approxMatches, "A: %d", code[1]);
      lcdClear(lcd);
      lcdPosition(lcd, 0, 0); lcdPuts(lcd, exactMatches);
      lcdPosition(lcd, 5, 0); lcdPuts(lcd, approxMatches);
  ...
  if (found) 
  {
    char stringAttempts[20];
    lcdClear(lcd);
    fprintf(stderr, "Success in %d try(s)", attempts);
    sprintf(stringAttempts, "SUCCESS IN %d TRY(S)\n", attempts);
    lcdPosition (lcd, 0, 0) ; 
    lcdPuts (lcd, stringAttempts) ;
